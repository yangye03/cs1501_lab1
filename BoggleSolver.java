...
public class BoggleSolver
{
		...
		for(int direction=0; direction<8; direction++){
			if(isValid(row, col, direction)){
				...
				if(res == 3){ //word and prefix
					...
					solve(nextCoords.row, nextCoords.col, depth+1 ); //search for words that have the prefix
				}
	...
	private boolean isValid(int row, int col, int direction){
		...
		//At an edge of the board
		if(coords.row<0 || coords.col<0 || coords.row >= theBoard.length || coords.col >= theBoard[0].length){
			return false;
		}
